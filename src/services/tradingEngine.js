...
/* =====================================================
   🔥 FIX 1: REAL PRICE UPDATE LOGIC
   ===================================================== */
const updateEventPrice = async (event, tradePrice) => {
    if (!event || !tradePrice) return;

    const IMPACT = 0.01; // price smoothness
    const delta = tradePrice * IMPACT;

    // PRICE FLUCTUATION LOGIC
    if (tradePrice >= event.currentYesPrice) {
        // YES demand ↑
    ...
    console.log(
        `[PRICE UPDATED] Event ${event._id} | YES ${event.currentYesPrice} | NO ${event.currentNoPrice}`
    );
};
...
const executeTrade = async (session, takerOrder, makerOrder, price, quantity) => {
    ...
    console.log(
        `[TRADE] ${quantity} @ ${price} | Taker ${takerOrder._id}`
    );
    ...
    sellerPos.quantity -= quantity;
    sellerPos.realizedProfit +=
        totalAmount - sellerPos.averagePrice * quantity;
    await sellerPos.save({ session });

    /* =====================================================
       🔥 FIX 2: PRICE UPDATE CALL
       ===================================================== */
...
export const settleEvent = async (eventId, result) => {
    ...
    try {
        ...
        for (const pos of positions) {
            const winnings =
                pos.side === result ? pos.quantity * 10 : 0;
            ...
            pos.realizedProfit +=
                winnings - pos.averagePrice * pos.quantity;
